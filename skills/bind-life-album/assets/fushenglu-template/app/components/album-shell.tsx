...
export function ContactNote({ contact }: { contact: AlbumContact }) {
  return (
    <aside className="contact-note absolute top-[34px] right-9 z-[9] w-40 origin-top-right rotate-[1.2deg] border border-[rgba(85,62,31,.25)] bg-[linear-gradient(145deg,#ded0b2,#bda984)] px-4 pt-[15px] pb-[14px] text-center text-[rgba(67,49,28,.82)] opacity-60 shadow-[0_12px_28px_rgba(0,0,0,.24),inset_0_0_0_1px_rgba(255,248,225,.18)] transition-[opacity,transform,box-shadow] duration-300 hover:translate-y-[-3px] hover:rotate-0 hover:opacity-[.78] focus-within:translate-y-[-3px] focus-within:rotate-0 focus-within:opacity-[.78] max-[760px]:top-[19px] max-[760px]:right-3 max-[760px]:scale-[.78]" aria-label={contact.label}>
      <strong className="block text-[13px] font-normal tracking-[.2em]">{contact.label}</strong>
      {contact.qrImageUrl && contact.supportUrl && (
        <a className="mx-auto mt-[11px] block w-[118px] text-inherit no-underline" href={contact.supportUrl} target="_blank" rel="noreferrer" aria-label={contact.qrLabel || "请造册人喝杯茶"}>
          <img className="block aspect-square w-full border-[5px] border-[rgba(246,239,220,.92)] object-contain shadow-[0_3px_10px_rgba(65,44,21,.14)] transition-transform duration-200 hover:-translate-y-0.5" src={contact.qrImageUrl} alt="付茶钱二维码" />
          <span className="mt-1.5 block text-[9px] leading-[1.4] tracking-[.16em] opacity-[.78]">{contact.qrLabel || "请造册人喝杯茶"}</span>
        </a>
      )}
      {contact.detail && <p className="my-2 text-[9px] leading-[1.7] tracking-[.11em] opacity-70">{contact.detail}</p>}
      {contact.url && <a className="grid min-h-11 place-items-center border border-[rgba(134,98,45,.58)] bg-[linear-gradient(135deg,#172b39,#0a151d)] px-2 py-[7px] text-[9px] tracking-[.18em] text-[#e4c98f] no-underline transition-colors hover:border-[rgba(181,141,72,.82)] hover:text-[#f4deb0]" href={contact.url} target="_blank" rel="noreferrer">{contact.actionLabel || "拜访造册人"}</a>}
    </aside>
  );
}
