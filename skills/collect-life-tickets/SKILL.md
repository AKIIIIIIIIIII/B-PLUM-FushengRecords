...
description: "把人生票根的同名 PNG 与 JSON 收进已有《浮生录》，保留原始文件，并按 ticketNumber 更新。适用于用户说“收录票根”“收入浮生录”“追加票根”“把这些票放进册子”时；建立空册请用 bind-life-album。"
---

# 收录票根

把已完成的人生票根完整收进现有藏本。每张票都必须有同名 PNG 与 JSON；只有 PNG 时，拒绝正式目录收录并列出缺失的 JSON，避免票据数据永久丢失。

## 选择票根来源

- 用户要收录当前对话刚完成的票根时，直接复用本轮已成功交付的 PNG 与 JSON，不再要求下载、重新上传或提供本地路径。
...
1. 确认已有藏本目录，并按上面的来源规则取得票根文件；仅在尚未提供来源时，才向用户索取附件或路径。
2. 运行，可重复提供输入：
