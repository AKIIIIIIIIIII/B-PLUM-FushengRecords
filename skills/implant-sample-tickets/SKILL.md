...
description: "批量生成完整的虚构样票，默认过去 5 张、未来 5 张且数量可调；可独立植入已有空册，也可为 create-fusheng-record 先准备票根文件。用于用户说“样票植入”“放些示例票”或“生成测试票根”时；真实个人经历使用 make-life-ticket。"
---
...
默认批量可能耗时较长；持续报告完成数量。失败项不得阻断已经完成的票，也不得被静默跳过。协调准备模式一张完整票都未生成时明确失败，让上游在造册前停止；独立植入模式最终汇总成功收录数，协调准备模式汇总成功准备数。
